function zordinator -d "Ultimate zord creation tool"
  ...
  set -gx keys 'fish' 'nvim' 'vim' 'ssh'  'gnupg' 'tilde' 'yamllint' 'starship' 'kitty' 'claude' 'work'
  set -gx linux_keys 'sway'
  set -gx osx_keys

  set -gx paths ~/.config/fish ~/.config/nvim ~/.vim ~/.ssh ~/.gnupg ~ ~/.config/yamllint ~/.config/starship ~/.config/kitty ~/.claude ~/workspace/work
